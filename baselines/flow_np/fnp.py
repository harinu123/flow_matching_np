...
class AttrDict(dict):
    """Tiny stand-in for attrdictionary.AttrDict.

    Allows dot-access (outs.loss) while behaving like a dict.
    """
...
class FNP(nn.Module):
    ...
    def forward(self, batch, num_samples=None, reduce_ll=True):
        ...
        if self.training:
            ...
            if reduce_ll:
                outs.tar_ll = None if log_p_y1 is None else log_p_y1.mean()
            else:
