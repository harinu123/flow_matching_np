...
class TNPA(TNP):
    ...
    def forward(self, batch, num_samples=None, reduce_ll=True):
        ...
        if self.sigma_bound > 0.0:
            std = self.sigma_bound + (1-self.sigma_bound) * F.softplus(std)
        else:
            std = torch.exp(std)
    ...
    def permute_sample_batch(self, xt, yt, num_samples, batch_size, num_target):
        ...
        dim_sample = torch.arange(num_samples, device=device).unsqueeze(-1).unsqueeze(-1).repeat((1,batch_size,num_target))
        dim_batch = torch.arange(batch_size, device=device).unsqueeze(0).unsqueeze(-1).repeat((num_samples,1,num_target))
    ...
    def predict(self, xc, yc, xt, num_samples=50, return_samples=False):
        ...
        yt_pred = torch.zeros((batch_size, num_target, yc.shape[2]), device=device)
