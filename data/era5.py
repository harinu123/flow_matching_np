...
era5_dataset_path = os.path.join(datasets_path, 'era5')
...
class ERA5Dataset(utils.Dataset):
    ...
    def __init__(self, mode: str = 'train', Min_n_cont=2, Max_n_cont=200, circular=True, normalize=True,
        ...
        path_to_nc_file = f"{era5_dataset_path}/{place}/{mode}_ERA5_{place}.nc"

        # Load the data as an xarray:
        # self.Y_data = xarray.open_dataset(path_to_nc_file).to_array()
        with xarray.open_dataset(path_to_nc_file) as dataset:
            self.Y_data = dataset.to_array()
