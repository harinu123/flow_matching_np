...
def img_to_task(img, num_ctx=None,
        max_num_points=None, ctx_mode=0, target_all=False, t_noise=None):
    ...
    if ctx_mode==1:
        idxs = torch.arange(num_points).to(img.device)[None].repeat(B, 1) # top half context
    else:
